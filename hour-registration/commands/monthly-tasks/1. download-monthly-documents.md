...
Use the Playwright MCP server. CLIENT is defined in `rules/context.md` (Companies section); keychain service names come from the Credentials section there. "Previous month" is computed in the **Europe/Amsterdam** timezone. All files are saved to `~/Downloads/{MM-YY}` (e.g. `~/Downloads/04-26` for April 2026); create the directory if it does not exist.
