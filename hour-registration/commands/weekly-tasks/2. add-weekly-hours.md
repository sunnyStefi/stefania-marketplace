...
| Variable | Eneco Fieldglass | Sourcelabs Administratie |
|---|---|---|
...
| `{keychain}` | `worksheet-fieldglass` | `worksheet-sourcelabs` |
| `{already-submitted}` | status is not `Draft` / `Not submitted` | any non-zero hours already entered, or week is locked/submitted |
| `{columns}` | Eneco hours column | Sourcelabs project, hours, and Thuiswerken Vergoeding columns |
